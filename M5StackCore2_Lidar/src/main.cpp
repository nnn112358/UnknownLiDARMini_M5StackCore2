...
void loop() {
  ...
  while (LIDARSerial.available() && bufferIndex < BUFFER_SIZE) {
    size_t bytesToRead = min(static_cast<size_t>(BUFFER_SIZE - bufferIndex),
                             min(static_cast<size_t>(LIDARSerial.available()),
                                 static_cast<size_t>(READ_CHUNK_SIZE)));
    size_t bytesRead = LIDARSerial.readBytes(buffer + bufferIndex, bytesToRead);
  ...
  while (packetStart + sizeof(LidarPacket_t) <= bufferIndex) {
    switch (state) {
      case STATE_FIND_HEADER:
        // ヘッダーを探す
        for (; packetStart + sizeof(header) <= bufferIndex; packetStart++) {
          if (memcmp(buffer + packetStart, header, sizeof(header)) == 0) {
      ...
      case STATE_READ_PAYLOAD:
        ...
        if (packetStart + sizeof(LidarPacket_t) <= bufferIndex) {
          memcpy(&packet, buffer + packetStart, sizeof(LidarPacket_t));
      ...
      case STATE_PROCESS_PACKET:
        ...
        if (degree_begin < 360 && degree_end < 360) {
          Serial.printf("%3drpm %5d - %5d\n",
                        convertSpeed(packet.rotation_speed), degree_begin,
                        degree_end);

          uint16_t map[16];
          uint16_t distances[16] = {
              packet.distance_0,  packet.distance_1,  packet.distance_2,
              packet.distance_3,  packet.distance_4,  packet.distance_5,
              packet.distance_6,  packet.distance_7,  packet.distance_8,
              packet.distance_9,  packet.distance_10, packet.distance_11,
              packet.distance_12, packet.distance_13, packet.distance_14,
              packet.distance_15};
  ...
  for (int i = 0; i < 359; i++) {
    // Serial.printf("%d %d %d\n", i, pointCloud[i].x, pointCloud[i].y);
    int x1 = pointCloud[i].x / v_Scale + 160;
    int y1 = pointCloud[i].y / v_Scale + 120;
    int x2 = pointCloud[i].x / v_Scale + 160;
    int y2 = pointCloud[i].y / v_Scale + 120;

    canvas.fillTriangle(160, 120, x1, y1, x2, y2, CYAN);
    // canvas.drawLine(160, 120, x1, y1, CYAN);
    // canvas.drawPixel(x1, y1, YELLOW);
  }
